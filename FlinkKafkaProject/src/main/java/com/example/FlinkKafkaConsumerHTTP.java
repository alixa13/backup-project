...
/**
 * Consumer for http.log records, processes all fields from Zeek HTTP logs,
 * sends for classification and anomaly detection.
 */
public class FlinkKafkaConsumerHTTP {
    private static final Logger LOGGER = Logger.getLogger(FlinkKafkaConsumerHTTP.class.getName());
    ...
    public static void main(String[] args) throws Exception {
            ...
            .map(new MapFunction<String, ZeekLogEvent>() {
                ...
                public ZeekLogEvent map(String value) throws Exception {
                    try {
                        ...
                        event.addField("method", safeGetString(root, "method"));
                        event.addField("host", safeGetString(root, "host"));
                        event.addField("uri", safeGetString(root, "uri"));
                        event.addField("referrer", safeGetString(root, "referrer"));
                        event.addField("user_agent", safeGetString(root, "user_agent"));
                        event.addField("status_code", safeGetInt(root, "status_code", 0));
                        event.addField("resp_mime_types", safeGetString(root, "resp_mime_types"));
        ...
        env.execute("Flink Zeek HTTP Log Processor");
    }
