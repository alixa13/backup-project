...
public final class ConnFeatureProcessFunction extends KeyedProcessFunction<SourceKey, NetworkEvent, FeatureVector> {
    ...
    public void processElement(NetworkEvent event, Context ctx, Collector<FeatureVector> out) throws Exception {
        ...
        ConnEvent conn = switch (event) {
            ...
            // conn-only operator, so reaching here is a wiring error too.
            case ModbusEvent ignored -> throw new IllegalStateException(
                "ConnFeatureProcessFunction received a ModbusEvent; the modbus chain is separate by design "
            ...
            case S7commEvent ignored -> throw new IllegalStateException(
                "ConnFeatureProcessFunction received an S7commEvent; the s7comm chain is separate by design "
