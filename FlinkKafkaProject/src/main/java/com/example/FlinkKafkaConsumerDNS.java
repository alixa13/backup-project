...
/**
 * Consumer for dns.log records, processes all fields from Zeek DNS logs,
 * sends for classification and anomaly detection.
 */
public class FlinkKafkaConsumerDNS {
    private static final Logger LOGGER = Logger.getLogger(FlinkKafkaConsumerDNS.class.getName());
    ...
    public static void main(String[] args) throws Exception {
            ...
            .map(new MapFunction<String, ZeekLogEvent>() {
                ...
                public ZeekLogEvent map(String value) throws Exception {
                    try {
                        ...
                        // Set DNS-specific fields
                        event.addField("query", safeGetString(root, "query"));
                        event.addField("qtype", safeGetInt(root, "qtype", 0));
                        event.addField("rcode", safeGetInt(root, "rcode", 0));
                        event.addField("answers", safeGetArrayAsString(root, "answers"));
                        event.addField("ttls", safeGetArrayAsString(root, "TTLs"));
        ...
        env.execute("Flink Zeek DNS Log Processor");
    }
    ...
    private static String safeGetArrayAsString(JsonNode root, String field) {
        JsonNode node = root.get(field);
