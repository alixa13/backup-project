...
// it never mutates ReferenceModbusEntityState itself. Groups A (current-event Modbus
...
// same event's own fields, and for resetting to ReferenceModbusEntityState.empty()
...
// reset `before` to ReferenceModbusEntityState.empty()
// (ReferenceModbusEntityState.resetForNewSegment), so prev_event_available,
...
public final class ReferenceModbusFeatureExtractor {
    ...
    public float[] extract(ModbusEvent event, ReferenceModbusEntityState before, ReferenceModbusEntityState after,
                            boolean newSegment) {
        // newSegment implies the caller already reset `before` to
        // ReferenceModbusEntityState.empty() ahead of this call -- see this class's
        ...
        if (newSegment && !(before.lastTs() == null && before.lastAddress() == null
            ...
            throw new IllegalArgumentException(
                ...
                + "ReferenceModbusEntityState.empty(); a non-empty before-state here would silently "
        ...
        if (prevAvailable) {
            ...
            // prevFunctionCode is always non-null here: ReferenceModbusEntityState.afterEvent
